...
package cn.onenet.protocol.nb.utils;
...
public final class HttpSendCenter {
  ...
  public static JSONObject post(String apiKey, String url, JSONObject body) {
    ...
    Request request =
        new Request.Builder().url(url).header("api-key", apiKey).post(requestBody).build();
    return handleRequest(request);
  }
  ...
  public static JSONObject put(String apiKey, String url, JSONObject body) {
    RequestBody requestBody = RequestBody.create(JSON, body.toString());
    Request request =
        new Request.Builder().url(url).header("api-key", apiKey).put(requestBody).build();
    return handleRequest(request);
  }
